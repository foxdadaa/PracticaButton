...
public class Calculator : MonoBehaviour {
    [SerializeField] private TMP_InputField _displayText;
    [SerializeField] private int _maxDisplayLength = 12;
    ...
    public void OnPressedButton(GameObject gameObject) {
        string gameObjectName = gameObject.name;
        if (gameObjectName.Contains("_")) {
            int number = int.Parse(gameObjectName.Substring(gameObjectName.LastIndexOf("_")+1));
            Debug.Log("number:" + number);
            OnPressedNumber(number);
        } else if (gameObjectName.Contains(":")) {
            string command = gameObjectName.Substring(gameObjectName.LastIndexOf(":")+1);
            ...
            if (command.Equals("On")) {
                ...
                OnPressedOperator(command);
            }
        }
    }
    public bool OnPressedOperator(string op) {
        switch (op) {
            case "+":
            case "-":
            case "÷":
        ...
        try {
            switch (currentOperator) {
                ...
                case "sqrt":
                    ...
                    break;
                case "pow":
                    Value = (decimal)Mathf.Pow((float)Value, 2);
                    break;
                default:
